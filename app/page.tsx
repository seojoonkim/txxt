...
const CheckIcon = ({color='#00D9A3',size=16}:{color?:string,size?:number}) => (
...
function TerminalCursor() {
  ...
  return <span style={{ opacity: on ? 1 : 0, color: '#00D9A3' }}>▋</span>;
}
...
function LayeredArchDiagram() {
  const agents = [
    ...
    { label: 'TradeAgent', color: '#00D9A3' },
    { label: 'DataBot', color: '#4285F4' },
    { label: 'LegalAI', color: '#4285F4' },
    { label: 'SearchAgent', color: '#00D9A3' },
  ];
  const chains = [
  ...
  const protocols = [
    { label: 'x402 Payments', color: '#00D9A3' },
    { label: 'ERC-8004 Identity', color: '#5B4FFF' },
    { label: 'PoAW Validation', color: '#00D9A3' },
  ];
  ...
  const ArrowConnector = () => (
    <div style={{ display: 'flex', justifyContent: 'center', padding: '8px 0' }}>
      <svg width="2" height="32" viewBox="0 0 2 32" fill="none">
        <line x1="1" y1="0" x2="1" y2="24" stroke="#00D9A3" strokeWidth="1.5" strokeDasharray="4,3"/>
        <path d="M-4 22L1 28L6 22" stroke="#00D9A3" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
      </svg>
    </div>
  );
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 0, width: '100%' }}>
      ...
      <div style={{
        ...
        boxShadow: '0 0 0 4px rgba(0,217,163,0.08), 0 4px 16px rgba(0,217,163,0.1)',
    ...
    <div style={{ background: '#FFFFFF', color: '#0D0D0D', fontFamily: 'Inter, system-ui, sans-serif', overflowX: 'hidden' }}>
        ...
        `}</style>
          <div style={{ maxWidth: 640, width: '100%', textAlign: 'center' as const }}>
            ...
            <p style={{
              ...
              <span style={{ color: '#00D9A3' }}>The agent economy runs on <strong>txxt</strong>.</span>
            </p>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'nowrap' as const }} className="hero-buttons">
              <Link href="/build" style={{
                ...
                boxShadow: '0 4px 14px rgba(0,217,163,0.25)',
              ...
              <Link href="https://docs.txxt.network" target="_blank" style={{
                ...
                color: '#00D9A3', fontWeight: 500, fontSize: 'clamp(13px, 3.5vw, 16px)',
                textDecoration: 'none',
                transition: 'all 0.2s ease',
                textAlign: 'center' as const,
                whiteSpace: 'nowrap' as const,
          ...
          <div style={{
            ...
            {[
              ...
              <div key={label} style={{ textAlign: 'center', flex: '1 1 0', minWidth: 0 }}>
                <div style={{ fontSize: 'clamp(13px, 2.5vw, 22px)', fontWeight: 800, color: '#00D9A3', fontFamily: mono, whiteSpace: 'nowrap' }}>{value}</div>
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: 'clamp(80px, 12vh, 140px) 24px', position: 'relative', zIndex: 1 }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ fontSize: 13, fontFamily: mono, fontWeight: 700, color: '#00D9A3', letterSpacing: '0.08em', marginBottom: 12 }}>x402 PROTOCOL — PAYMENT</div>
      ...
      <section style={{ background: '#F8F8F8', borderTop: '1px solid rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ maxWidth: 900, margin: '0 auto', padding: '40px 24px 0' }}>
          {[
            { icon: (color: string) => <svg width="56" height="56" viewBox="0 0 56 56" fill="none"><rect width="56" height="56" rx="14" fill={color}/><rect x="12" y="18" width="32" height="20" rx="4" stroke="white" strokeWidth="2.5"/><path d="M12 26h32" stroke="white" strokeWidth="2.5"/><rect x="16" y="30" width="8" height="3" rx="1.5" fill="white"/><rect x="27" y="30" width="5" height="3" rx="1.5" fill="white"/></svg>, word: 'x402 Native', title: 'Payments as infrastructure.', desc: 'Implementing x402 on Ethereum alone requires custom contracts, unpredictable gas, and weeks of testing. Through txxt, agent.pay() works on ETH, SOL, Base, Polygon — same call, same $0.0003 cost, always.', color: '#00D9A3', num: '01' },
            ...
            { icon: (color: string) => <svg width="56" height="56" viewBox="0 0 56 56" fill="none"><rect width="56" height="56" rx="14" fill={color}/><path d="M28 12L34 22H44L36 29L39 40L28 33L17 40L20 29L12 22H22L28 12Z" stroke="white" strokeWidth="2.5" strokeLinejoin="round"/></svg>, word: 'Atomic Integration', title: 'One transaction does both.', desc: 'Imagine: your agent verifies a counterpart\'s identity, then sends payment — and the identity check passes but the payment fails. Now you\'ve trusted an agent you never paid. txxt makes identity + payment atomic. Both succeed or neither does.', color: '#00D9A3', num: '03' },
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: 'clamp(80px, 10vw, 140px) 24px' }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 64 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
              ...
              <div style={{ padding: '12px 20px', borderRadius: 10, background: 'rgba(0,217,163,0.08)', fontFamily: mono, fontSize: 11, fontWeight: 700, color: '#00D9A3', letterSpacing: '0.1em', textTransform: 'uppercase' as const }}>
                With txxt
              </div>
            </div>
            {[
              ...
              { area: 'Agent Payments', diy: 'Write x402 adapter per chain + bridge SDK + USDC routing. Rewrite when specs update.', txxt: 'txxt.pay() — $0.0003 fixed, settles on whichever chain you choose', color: '#00D9A3' },
              { area: 'Work Verification', diy: 'Build custom oracle, reputation aggregator, and dispute resolution from scratch.', txxt: 'PoAW built-in — self-attest, peer-verify, protocol-confirm. Automatic reputation.', color: '#00D9A3' },
              ...
              { area: 'Onboarding Time', diy: '2-6 weeks per chain. Add a new chain? Start over from scratch.', txxt: 'Under 5 minutes — any chain, any agent framework. New chains auto-supported.', color: '#00D9A3' },
            ].map(row => (
              <div key={row.area} style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                ...
                {/* txxt */}
                <div style={{ padding: '20px', borderRadius: 12, background: 'rgba(0,217,163,0.04)', border: '1px solid rgba(0,217,163,0.15)' }}>
                  <div style={{ fontSize: 11, fontFamily: mono, color: '#888888', fontWeight: 600, marginBottom: 10, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>{row.area}</div>
                  <div style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                    <svg width="14" height="14" viewBox="0 0 14 14" fill="none" style={{ flexShrink: 0, marginTop: 3 }}>
                      <path d="M2 7l3.5 3.5 6.5-7" stroke="#00D9A3" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"/>
                    </svg>
...
// ... 100+ lines of glue code per chain`}</pre>
            ...
            {/* txxt */}
            <div style={{ borderRadius: 16, border: '2px solid #00D9A3', overflow: 'hidden', boxShadow: '0 4px 24px rgba(0,217,163,0.12)' }}>
              <div style={{
                padding: '12px 20px', background: 'rgba(0,217,163,0.06)',
                borderBottom: '1px solid rgba(0,217,163,0.15)',
                display: 'flex', justifyContent: 'space-between', alignItems: 'center',
              }}>
                <span style={{ fontSize: 13, fontFamily: mono, fontWeight: 700, color: '#00D9A3' }}>txxt Middleware</span>
                <span style={{ fontSize: 11, fontFamily: mono, color: '#00D9A3', fontWeight: 600 }}>5 lines · any chain</span>
              </div>
              <div style={{ padding: '28px 20px', fontFamily: mono, fontSize: 'clamp(12px, 1.3vw, 14px)', lineHeight: 2.4, background: '#FFFFFF', overflow: 'auto' }}>
                <div><span style={{ color: '#5B4FFF' }}>import</span>{' '}<span style={{ color: '#0D0D0D' }}>{'{ txxt }'}</span>{' '}<span style={{ color: '#5B4FFF' }}>from</span>{' '}<span style={{ color: '#00D9A3' }}>{`'@txxt/sdk'`}</span>;</div>
                <div><span style={{ color: '#5B4FFF' }}>const</span>{' '}<span style={{ color: '#FB923C', fontWeight: 600 }}>agent</span>{' = '}<span style={{ color: '#0D0D0D' }}>txxt</span>.<span style={{ color: '#00D9A3', fontWeight: 600 }}>connect</span>{'({ apiKey, chains: ['}<span style={{ color: '#00D9A3' }}>{`'eth'`}</span>{', '}<span style={{ color: '#00D9A3' }}>{`'sol'`}</span>{', '}<span style={{ color: '#00D9A3' }}>{`'base'`}</span>{'] });'}</div>
                <div><span style={{ color: '#5B4FFF' }}>await</span>{' '}agent.<span style={{ color: '#00D9A3', fontWeight: 600 }}>register</span>{'({ capabilities: ['}<span style={{ color: '#00D9A3' }}>{`'translate'`}</span>{'] });'}</div>
                <div><span style={{ color: '#5B4FFF' }}>await</span>{' '}agent.<span style={{ color: '#00D9A3', fontWeight: 600 }}>pay</span>{'(url, '}<span style={{ color: '#00D9A3' }}>{`'$0.01'`}</span>{', { chain: '}<span style={{ color: '#00D9A3' }}>{`'base'`}</span>{' });'}</div>
                <div><span style={{ color: '#94A3B8' }}>{'// done. identity + payment. any chain.'}</span></div>
              </div>
              <div style={{
                padding: '16px 20px', background: 'rgba(0,217,163,0.04)',
                borderTop: '1px solid rgba(0,217,163,0.1)',
                fontSize: 13, color: '#00D9A3', fontFamily: mono, fontWeight: 600,
              }}>
...
//   uptime: 99.9 }`}
          ...
          <div style={{
            ...
            <p style={{ fontSize: 13, letterSpacing: '0.12em', color: '#00D9A3', fontFamily: mono, marginBottom: 16, fontWeight: 600 }}>REPUTATION</p>
            <h3 style={{ fontSize: 'clamp(24px, 4vw, 36px)', fontWeight: 700, marginBottom: 16, letterSpacing: '-0.03em', lineHeight: 1.2 }}>
        ...
        <div style={{
          ...
          <div style={{
            ...
            <div style={{
              fontSize: 'clamp(80px, 12vw, 160px)', fontWeight: 900,
              color: 'rgba(0,217,163,0.06)', fontFamily: mono,
              lineHeight: 1, marginBottom: 24, letterSpacing: '-0.05em',
            }}>03</div>
            <p style={{ fontSize: 13, letterSpacing: '0.12em', color: '#00D9A3', fontFamily: mono, marginBottom: 16, fontWeight: 600 }}>VALIDATION</p>
            <h3 style={{ fontSize: 'clamp(24px, 4vw, 36px)', fontWeight: 700, marginBottom: 16, letterSpacing: '-0.03em', lineHeight: 1.2 }}>
...
// ✓ triple-verified`}
      ...
      <section style={{
        ...
        <div style={{
          position: 'absolute', inset: 0, pointerEvents: 'none',
          background: 'radial-gradient(ellipse 40% 50% at 50% 50%, rgba(0,217,163,0.03) 0%, transparent 70%)',
        }} />
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: 'clamp(80px, 12vh, 140px) 24px', position: 'relative', zIndex: 1 }}>
          ...
          <h2 style={{
            ...
            No token.<br />
            <span style={{ fontSize: '0.85em', color: '#00D9A3' }}>Stable gas.</span>
          </h2>
          ...
          <div style={{
            ...
            <input
              ...
              style={{ width: '100%', accentColor: '#00D9A3', marginBottom: 16 }}
            />
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', gap: 16 }}>
              ...
              <div style={{ flex: 1, textAlign: 'center', padding: '12px', borderRadius: 8, background: 'rgba(0,217,163,0.05)', border: '1px solid rgba(0,217,163,0.1)' }}>
                <div style={{ fontSize: 11, color: '#888', fontFamily: mono, marginBottom: 4 }}>txxt</div>
                <div style={{ fontSize: 'clamp(18px, 3vw, 24px)', fontWeight: 800, color: '#00D9A3', fontFamily: mono }}>${(txCount * 0.0003).toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div>
              </div>
            </div>
            <p style={{ fontSize: 'clamp(14px, 2vw, 16px)', color: '#00D9A3', fontWeight: 700, fontFamily: mono, marginTop: 12, textAlign: 'center' }}>
          ...
          <div style={{
            ...
            <div style={{
              ...
              <div style={{ padding: '16px 20px', fontSize: 13, fontFamily: mono, color: '#00D9A3', fontWeight: 700, letterSpacing: '0.05em', textAlign: 'center' }}>txxt</div>
            </div>
            {[
              ...
              <div key={i} style={{
                ...
                <div style={{ padding: '14px 20px', fontSize: 13, color: '#00D9A3', fontWeight: 600, textAlign: 'center' }}>
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '0 24px' }}>
          ...
          <div style={{ position: 'relative', maxWidth: 700, margin: '0 auto 64px', aspectRatio: '1', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            ...
            <div style={{
              ...
              background: '#00D9A3', display: 'flex', alignItems: 'center', justifyContent: 'center',
              boxShadow: '0 0 40px rgba(0,217,163,0.3)',
            }}>
            ...
            {[
              ...
              { label: 'x402', color: '#00D9A3', angle: 135, desc: 'Payments' },
              { label: 'SDK', color: '#5B4FFF', angle: 180, desc: '@txxt/sdk' },
              { label: 'CLI', color: '#00D9A3', angle: 225, desc: 'Terminal' },
              ...
              {[0, 45, 90, 135, 180, 225, 270, 315].map((angle) => {
                ...
                return (
                  <line key={angle} x1="50" y1="50" x2={x2} y2={y2}
                    stroke="rgba(0,217,163,0.2)" strokeWidth="0.5" strokeDasharray="2,2" />
                );
              })}
            </svg>
          </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 16 }}>
            {[
              ...
              { label: 'x402', fullName: 'Agent Payment Protocol', by: 'Built-in', desc: 'HTTP-native payment protocol for agents. Pay per call, per result, or per task — settled on any chain for $0.0003.', color: '#00D9A3' },
              { label: 'CLI', fullName: 'Command Line Interface', by: 'txxt', desc: 'Deploy and manage agents from your terminal. txxt register, txxt pay, txxt identity — the fastest path from idea to deployed agent.', color: '#00D9A3' },
  ...
  task hire_flight_agent(destination) {
    ...
    rate(agent, score: 95, proof: result.receipt)
                ...
                style={{
                  ...
                  borderRadius: 6,
                  padding: '4px 10px',
                  color: copied ? '#00D9A3' : 'rgba(255,255,255,0.5)',
                  fontSize: 12,
                  fontFamily: mono,
                  cursor: 'pointer',
                  transition: 'all 0.2s ease',
            ...
            <div style={{ padding: 'clamp(24px, 4vw, 40px)' }}>
              <pre style={{ fontSize: 'clamp(12px, 1.3vw, 14px)', fontFamily: mono, lineHeight: 2, margin: 0, overflowX: 'auto' as const }}>
                <code>
                  ...
                  <span style={{ color: '#00D9A3' }}>TravelPlanner</span>{' '}
                  ...
                  <span style={{ color: '#00D9A3' }}>80</span>{' '}
                  ...
                  <span style={{ color: '#00D9A3' }}>discover</span>
                  ...
                  <span style={{ color: '#00D9A3' }}>85</span>{' '}
                  ...
                  <span style={{ color: '#00D9A3' }}>delegate</span>
                  ...
                  {'    '}<span style={{ color: '#00D9A3' }}>rate</span>
                  <span style={{ color: 'rgba(255,255,255,0.75)' }}>(agent, score: </span>
                  <span style={{ color: '#00D9A3' }}>95</span>
      ...
      <section style={{
        ...
        <div style={{ maxWidth: 1300, margin: '0 auto', padding: '0 24px', position: 'relative', zIndex: 1 }}>
          <h2 style={{
            ...
            <span style={{ color: '#00D9A3' }}>every</span> blockchain.
          </h2>
